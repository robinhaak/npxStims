...
%based on Jorrit Montijn's Acquipix 'RunDriftingGratings'
...
intStimSet = 1;% 1=two orientations (0 and 90), optogenetic stim on half of the trials
boolUseSGL = true;
boolUseNI = true;
boolDebug = false;
...
if exist('sExpMeta','var')
	...
	if isfield(sExpMeta,'dblOptoMultiplier'),dblOptoMultiplier=sExpMeta.dblOptoMultiplier;end
...
if ~exist('sStimParamsSettings','var') || isempty(sStimParamsSettings) || ~(strcmpi(sStimParamsSettings.strStimType,'SquareGrating') || ~strcmpi(sStimParamsSettings.strStimType,'SineGrating'))
	...
	sStimParamsSettings.strStimType = 'SineGrating';
	...
	sStimParamsSettings.vecSpatialFrequencies = 0.1; %Spat Frequency in cyc/deg 0.08
	sStimParamsSettings.vecTemporalFrequencies = 1; %NB this is only for varying the phase of the grating; Temporal frequency in cycles per second (0 = static gratings only)
	
	%opto stim parameters
	%-optogenetic stimulus on half of the trials (for each orientation,random)
	%-we have only two NI output channels, so laser will be INSTEAD of the sync light in the eye tracking video
	%-duration of the optogenetic stimulus == duration of the stimulus
...
boolUseOpto = true;
if boolUseOpto
	intStimTypes = 2*structEP.intStimTypes;
end

for intRep = 1:structEP.intNumRepeats
	%randomized order
	structEP.vecTrialStimTypes = [structEP.vecTrialStimTypes randperm(intStimTypes)];
end
structEP.intTrialNum = numel(structEP.vecTrialStimTypes);
structEP.vecOptoOn = structEP.vecTrialStimTypes > structEP.intStimTypes; %indices for opto trials
structEP.vecTrialStimTypes(structEP.vecOptoOn) = structEP.vecTrialStimTypes(structEP.vecOptoOn)-structEP.intStimTypes;
...
if boolUseNI
	...
	if exist('objDaqOut','var') && ~isempty(objDaqOut)
		try
			%turns leds on
			stop(objDaqOut);
			outputData1 = dblOptoMultiplier*cat(1,linspace(3, 3, 200)',linspace(0, 0, 50)');
...
try
	...
	for intThisTrial = 1:structEP.intTrialNum
		...
		dblPostBlank = dblEndSecs - dblStimOffSecs;
		
		%check if opto is on during this trial
		%-fs = 1000Hz
		boolOptoOn = structEP.vecOptoOn(intThisTrial);
		if boolUseNI
			stop(objDaqOut);
			if boolOptoOn
				outputData1 = dblOptoMultiplier*cat(1,linspace(3, 3, 1000)',linspace(0, 0, 100)');
			else
				outputData1 = dblOptoMultiplier*cat(1,linspace(0, 0, 1100)');
			end
			
			outputData2 = dblPupilLightMultiplier*linspace(3, 3, 1100)';
			queueOutputData(objDaqOut,[outputData1 outputData2]);
			prepare(objDaqOut);
		end
		...
		boolFirstFlip = false;
		refTimeLocal = tic;
		intFrame = randperm(length(vecTex),1);
	...
	%% save data
	%save data
	structEP.TemporalFrequency = zeros(size(structEP.SpatialFrequency));
