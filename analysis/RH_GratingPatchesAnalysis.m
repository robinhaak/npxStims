...
%Robin Haak,  June 2023

vecChs = 1:385; %inputs channels to plot (1 = bottom Ch?)
...
%% plot data
%interpolate
vecX_pix_interp = linspace(vecX_pix(1),vecX_pix(end),53); %53
vecY_pix_interp = linspace(vecY_pix(1),vecY_pix(end),29); %29
...
for intCh = vecChs
	if max(max(matAvgRespAll(:,:,intCh))) >  2
% 				matAvgRespAll_interp = interp2(matAvgRespAll(:,:,intCh),2);
