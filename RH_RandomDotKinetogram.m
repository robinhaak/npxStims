...
if ~exist('sStimParamsSettings','var') || isempty(sStimParamsSettings) || ~strcmpi(sStimParamsSettings.strStimType,'RandomDotKinematogram')
    ...
    coherenceSetLabel = 'Coherence Set (4 directions, 6 coherence levels)';

    choice = questdlg('Choose a Stimulus Set:', ...
                      'Stimulus Configuration', ...
                      tuningSetLabel, coherenceSetLabel, tuningSetLabel); % 'Tuning Set' is default
    switch choice
        case tuningSetLabel
        ...
        case coherenceSetLabel
            ...
            sStimParamsSettings.vecCoherenceLevels = [0.03 0.06 0.12 0.24 0.48 0.96]; % All 6 coherence levels
            fprintf('Selected: Coherence Set (4 directions, 6 coherences)\n');
        otherwise
...
try
    ...
    uniqueKeysForPrecomputation = unique(trialList(:,1:3), 'rows'); % Get unique combinations of (coh, dir, seed)
    for i = 1:size(uniqueKeysForPrecomputation,1)
        ...
        key = sprintf('coh_%.2f_dir_%.0f_seed_%d', coh, dirDeg, seedIdx);
