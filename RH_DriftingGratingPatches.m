...
%RH_DRIFTINGGRATINGSPATCHES
%Display patches of gratings, drifting in the cardinal to directions to,
%e.g., map receptive fields
%
%Robin Haak, 2022
...
if ~exist('sStimParamsSettings','var') || isempty(sStimParamsSettings) || ~(strcmpi(sStimParamsSettings.strStimType,'SquareGrating') || ~strcmpi(sStimParamsSettings.strStimType,'SineGrating'))
    ...
    sStimParamsSettings.intUseScreen = 0; % 2; %which screen to use
    ...
    sStimParamsSettings.dblStimulusSize_deg = 16; %deg; (approximate) size of grating patches
    ...
    sStimParamsSettings.dblSecsStimulusDuration = 0.5; %0.5; %s
    sStimParamsSettings.dblSecsStimulusInitialBlank = 3; %s
    sStimParamsSettings.dblSecsStimulusPreBlank = 0.25; %s
    sStimParamsSettings.dblSecsStimulusPostBlank = 0.5; %s
    sStimParamsSettings.dblSecsStimulusEndBlank = 5; %s
...
try
    ...
    %create single grating
    vecX = meshgrid(-intFullTexSize_pix:intFullTexSize_pix+intPixelsPerCycle,1);
    matGrating = sStimParams.intBackground+(255-sStimParams.intBackground)*cos(dblSpatialFrequency_rad*vecX);
    intGratingTex = Screen('MakeTexture',ptrWindow,matGrating);
    ...
    vecPosY = vecPosY + ((sStimParams.intScreenWidth_pix-(max(vecPosY)+sStimParams.intStimulusSize_pix))/2);
    ...
    %create randomized vector w/drifting directions
    vecDirections = repmat(sStimParams.vecDirections,[1 floor(intTotalTrials/length(sStimParams.vecDirections))]);
    vecDirections = [vecDirections vecDirections(1:(intTotalTrials-length(vecDirections)))];
    vecDirections = vecDirections(randperm(length(vecDirections)));

    %% build structEP
    %stimulus timing info
    initialBlank = sStimParams.dblSecsStimulusInitialBlank;
    trialDur = sStimParams.dblSecsStimulusPreBlank+sStimParams.dblSecsStimulusDuration+sStimParams.dblSecsStimulusPostBlank;
    endBlank = sStimParams.dblSecsStimulusEndBlank;
    ...
    structEP.dblOrientation = nan(1,structEP.intTrialNum);
    structEP.vecTrialStartSecs = initialBlank:trialDur:(totalLength-endBlank-1);
    structEP.vecTrialStimOnSecs = structEP.vecTrialStartSecs+sStimParams.dblSecsStimulusPreBlank;
    structEP.vecTrialStimOffSecs = structEP.vecTrialStimOnSecs+sStimParams.dblSecsStimulusDuration;
    structEP.vecTrialEndSecs = structEP.vecTrialStimOffSecs+sStimParams.dblSecsStimulusPostBlank;
    ...
    fprintf('Starting initial blank (dur=%.3fs) [%s]\n',sStimParams.dblSecsStimulusInitialBlank,getTime);
    dblInitialBlankDur = 0;
    while dblInitialBlankDur < (sStimParams.dblSecsStimulusInitialBlank-dblStimFrameDur)
        %do nothing
    ...
    while intThisTrial <= intTotalTrials && ~CheckEsc()
        ...
        dblAngle = vecDirections(intThisTrial)+180;
        if dblAngle >= 360, dblAngle = dblAngle-360; end

        %add to structEP
        structEP.vecDstRect(:,intThisTrial) = vecDstRect;
        structEP.dblOrientation(intThisTrial) = vecDirections(intThisTrial);
        ...
        intFrame = 1;
        refTimeLocal = tic;
        while toc(refTime)<(sStimParams.dblSecsStimulusDuration-2*dblStimFrameDur)
    ...
    %show trial summary
    fprintf('Finished experiment & data saving at [%s], waiting for end blank (dur=%.3fs)\n',getTime,sStimParams.dblSecsStimulusEndBlank)

    %% wait end-blanking
    dblEndBlankDur = 0;
    while dblEndBlankDur < sStimParams.dblSecsStimulusEndBlank
        %do nothing
        Screen('FillRect',ptrWindow, sStimParams.intBackground);
