% STRUCTEP = RH_GRATINGSPATCHESBIG()
% show patches of (drifting) gratings at different locations on the screen the screen
%
% Robin Haak
%
% history:
%   - created 25 Sep 2023 from RH_GRATINGPATCHES() for ease of use
% 28 Feb 2025
...
strHostAddress = '192.87.11.8'; %'192.87.11.84'; %'192.87.11.84'; %'192.87.10.228'; %'192.87.10.160'; %'192.87.11.133'; %default host address
objDaqOut = [];
...
if ~exist('sStimParamsSettings','var') || isempty(sStimParamsSettings) || ~(strcmpi(sStimParamsSettings.strStimType,'SquareGrating') || ~strcmpi(sStimParamsSettings.strStimType,'SineGrating'))
    ...
    sStimParamsSettings.dblScreenDistance_cm = 31; %32 %35; %43; %15; %23; % cm; measured [~23]

    %screen variables
    sStimParamsSettings.intCornerTrigger = 1; %1; %2; % integer switch; 0=none,1=upper left, 2=upper right, 3=lower left, 4=lower right
    ...
    %stimulus parameters
    sStimParamsSettings.dblStimulusSize_deg = 8; %15; %6 %deg; (approximate) size of grating patches
    ...
    sStimParamsSettings.dblTemporalFrequency = 2; %temporal frequency in cycles per second
...
%    sStimParams.dblPixelsPerDeg = sStimParams.intScreenHeight_pix/sStimParams.dblScreenHeight_deg; %to make it identical to RH_ONEDOTTWODOTS()
	sStimParams.dblPixelsPerDeg = sStimParams.intScreenWidth_pix/sStimParams.dblScreenWidth_deg;
